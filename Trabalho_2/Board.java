...
public class Board {
	...
	public boolean boardPlace (Tiles inTile, Player player, Board table, int indexTile, int nextPlayer) {
		...
		if (tileOnTable.size() != 0) 
		{
			...
			//este é para o caso em que as peças do topo e fundo do tabuleiro sao iguais a um número
			if (inTile.checkIn(topTile.getTopNumber()) && inTile.checkIn(botTile.getBotNumber()) && nextPlayer == 0){
				System.out.println("The tile you picked can be placed either on top tile and the bot tile.\n Where do you want to place it?\n 0) Top\n 1) Bot" );
					//enqunato a peça nao for colocada, o loop pede uma entrada correta do utilizador
				while(!tilePlaced) {
						optionTile = scan.nextLine().toUpperCase();
						
						//coloca a peça no topo
						if(optionTile.equals("0")) {
							...
							else        //caso seja necessário a peça é rotacionada
							{
								tileOnTable.add(0, inTile.rotateTile());
								tilePlaced = true;//loop é quebrado
							}
							
						} else if(optionTile.equals("1")) {
							//coloca a peça no fundo do tabuleiro
			...
			} else if (inTile.checkIn(topTile.getTopNumber()) && !tilePlaced){
			...
			else if (inTile.checkIn(botTile.getBotNumber()) && !tilePlaced) 
